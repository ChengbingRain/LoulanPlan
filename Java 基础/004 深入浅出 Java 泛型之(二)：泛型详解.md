...
`tMerge` 方法虽使用泛型 `T` 定义参数，但其本质上仍为普通方法，因为此处的 `T` 是泛型类 `Test<T>` 的类型形参。
...
注意上面并没有为父类 `Father` 指定类型，甚至连泛型符号 `<>` 都没有，但程序依然可以正常运行，只是编译阶段编译器会警告“使用了未经检查或不安全的操作”（可忽略）。
